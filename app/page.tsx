...
import VictoryDisplay from "./components/Victory/VictoryDisplay";
...
export default function Home() {
    ...
    const loadPlayerData = async () => {
      try {
        ...
        const pageRes = await fetch(
          `${API_URL}/pages/${player.current_page_id ?? player.currentPageId}`
        );
  ...
  const applyChoice = async (nextPageId: number) => {
    ...
    const res = await fetch(
      `${API_URL}/players/${loadPlayer.id}/choice`,
      {
        method: "POST",
        headers: {
  ...
  const handleRollDice = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/players/${loadPlayer.id}/roll-dice`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      <Modal isOpen={!!combatVictory} title="🎉 Victoire !" onClose={() => setCombatVictory(null)}>
        {combatVictory && (
          <>
            ...
            <button className={s.continueButton} onClick={handleContinueAfterVictory}>
